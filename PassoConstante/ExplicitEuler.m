function [u, info] = ExplicitEuler(f, tSpan, u0, n)
% [u, info] = ExplicitEuler(f, tSpan, u0, n)
% Executa o método numérico de Euler Explícito.
% INPUTS:
...
%     (não inclui etapas preparatórias, tais como as alocações).
  ...
  for i = 2 : n
    ...
    u(:, i) = ExplicitEulerStep(f, t(i-1), u(:, i-1), h);
    % A função f é chamada apenas uma vez no método de Euler Explícito.
    info.nEvals += 1;
  endfor
