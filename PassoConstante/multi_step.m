...
%     .telapsed = tempo de execução total em milissegundos.
  ...
  for i = 2 : min(s, n)
    ...
    sol.fevals += fevals_per_step_simple + 1;
  endfor
  ...
  for i = s+1 : n
    ...
    if method_multi == 'AM'
      last_f = shift(last_f, -1);
      sol.u(i,:) = AdamsBashforthStep(f, last_f, sol.u(i-1,:), h, s);
    ...
    sol.u(i,:) = step_multi(f, last_f, sol.u(i-1,:), h, s);
    if method_multi == 'AB'
    ...
    sol.fevals += fevals_per_step_multi;
  endfor
