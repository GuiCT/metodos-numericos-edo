function [u, info] = MidpointMethod(f, tSpan, u0, n)
% [u, info] = MidpointMethod(f, tSpan, u0, n)
% Executa o método numérico do ponto médio.
% INPUTS:
...
%     (não inclui etapas preparatórias, tais como as alocações).
  ...
  for i = 2 : n
    ...
    u(:, i) = MidpointMethodStep(f, t(i-1), u(:, i-1), h);
    % A função f é chamada duas vezes no método do ponto médio.
    info.nEvals += 2;
  endfor
