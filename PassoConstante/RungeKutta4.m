function [u, info] = RungeKutta4(f, tSpan, u0, n)
% [u, info] = RungeKutta4(f, tSpan, u0, h)
% Executa o método numérico de Runge-Kutta de quarta ordem (original).
% INPUTS:
...
%     (não inclui etapas preparatórias, tais como as alocações).
  ...
  for i = 2 : n
    ...
    u(:, i) = RungeKutta4Step(f, t(:, i-1), u(:, i-1), h);
    % A função f é chamada quatro vezes no método RK4.
    info.nEvals += 4;
  endfor
